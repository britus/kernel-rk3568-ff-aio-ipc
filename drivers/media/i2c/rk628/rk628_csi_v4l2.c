...
#define RK628_CSI_LINK_FREQ_HIGH	350000000
...
__retry:
	...
	//printk( "[zyk debug] this mode: %d x %d\n", bt->hfrontporch + bt->hsync + bt->width + bt->hbackporch, bt->vfrontporch + bt->vsync + bt->height + bt->vbackporch);
	return 0;
...
/*zyk debug, fix the aduio issue when the resulotion changing*/
static int rk628_check_resulotion_change(struct v4l2_subdev *sd)
{
	u32 val, ret;
	struct rk628_csi *csi = to_csi(sd);
	u32 htotal, vtotal;
	static u32 old_htotal =2200;
	static u32 old_vtotal= 1125;
	...
	// printk( "[zyk debug] new mode: %d x %d\n", htotal, vtotal);
	// printk("[zyk debug] old mode: %d x %d\n", old_htotal, old_vtotal);
...
extern int firefly_rk628_notifier_call_chain(unsigned long val, void *ptr);
static void rk628_hdmirx_vid_enable(struct v4l2_subdev *sd, bool en)
{
	struct rk628_csi *csi = to_csi(sd);
	int ret;
	...
	if (en) {
		...
		ret = firefly_rk628_notifier_call_chain(1, NULL);
		
        // if(ret)
        //         printk("[zyk debug] %s: rk628 notifier call chain failed\n",__func__);
		// else			  
		// 		printk("[zyk debug] %s excuse notifier chain\n",__func__);
		...
		ret = firefly_rk628_notifier_call_chain(0, NULL);
	}
}
