...
void rk628_dsi_pre_enable(struct rk628_dsi *dsi)
{
	u32 val;
	struct rk628 *rk628 = dsi->rk628;
...
void rk628_dsi_set_vid_mode(struct rk628_dsi *dsi)
{
	unsigned int lanebyteclk = (dsi->lane_mbps * 1000L) >> 3;
	unsigned int dpipclk;
	...
	dsi_write(rk628, 0, DSI_VID_HSA_TIME, VID_HSA_TIME(hsa_time));
	...
	dsi_write(rk628, 0, DSI_VID_VSA_LINES, vsa);
...
void rk628_dsi_set_cmd_mode(struct rk628_dsi *dsi)
{
	struct rk628 *rk628 = dsi->rk628;
