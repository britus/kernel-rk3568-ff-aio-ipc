...
static void enable_bt1120tx(struct v4l2_subdev *sd)
{
	struct rk628_bt1120 *bt1120 = to_bt1120(sd);
	u8 video_fmt;
	u8 lanes = bt1120->bt1120_lanes_in_use;
	u8 lane_num;
	u8 dphy_lane_en;
	u32 wc_usrdef, val = 0;
	int avi_rdy;
	...
	wc_usrdef = bt1120->timings.bt.width * 2;
...
#else
	if (!v4l2_match_dv_timings(&bt1120->timings, &timings, 0)) {
#endif
...
#else
static void rk628_bt1120_irq_poll_timer(unsigned long arg)
{
	struct rk628_bt1120 *bt1120 = (struct rk628_bt1120 *)arg;
				  ...
				  struct rkmodule_inf *inf)
{
	...
	strlcpy(inf->base.sensor, RK628_BT1120_NAME, sizeof(inf->base.sensor));
	strlcpy(inf->base.module, rk628_bt1120->module_name,
		sizeof(inf->base.module));
	strlcpy(inf->base.lens, rk628_bt1120->len_name, sizeof(inf->base.lens));
}
...
static const struct v4l2_subdev_video_ops rk628_bt1120_video_ops = {
	...
	.g_mbus_config = rk628_bt1120_g_mbus_config,
			  ...
			  const struct i2c_device_id *id)
{
...
#endif
#else
	v4l2_subdev_init(sd, &rk628_bt1120_ops);
#endif
	...
	err = media_entity_init(&sd->entity, 1, &bt1120->pad, 0);
#endif
	if (err < 0) {
...
#else
		bt1120->timer.data = (unsigned long)bt1120;
		bt1120->timer.function = rk628_bt1120_irq_poll_timer;
#endif
