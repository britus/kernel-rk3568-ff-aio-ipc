...
static int himax_palm_detect(uint8_t *buf)
{
	...
	D("%s: HX_PALM_REPORT_loopi=%d,base=%x,X=%x,Y=%x,W=%x\n",
			...
			size_t len, loff_t *data)
{
	...
	if (buf[0] == 'r') {
		...
		D("%s: Start to read chip test data.\n", __func__);
	}	else {
		chip_test_r_flag = false;
		D("%s: Back to do self test.\n", __func__);
	}
		...
		size_t len, loff_t *pos)
{
	...
	D("%s: himax_GESTURE_store= %s, len = %d\n", 
		__func__, buf, (int)len);
	...
	for (i = 0; i < len; i++) {
		if (buf[i] == '0' && j < GEST_SUP_NUM) {
			...
			D("%s: Not 0/1 or >=GEST_SUP_NUM : buf[%d] = %c\n",
				__func__, i, buf[i]);
	}
		...
		size_t len, loff_t *pos)
{
	...
	if (buf[0] == '0' && ts->SMWP_enable == 1) {
		...
		D("%s: SMWP is disable, not supprot to ctrl p-sensor.\n",
			__func__);
	} else
		return -EINVAL;
...
#if defined(__EMBEDDED_FW__)
		image = &g_embedded_fw;
		W("%s: Not find FW in userspace, use embedded FW(size:%zu)",
			__func__, g_embedded_fw.size);
#else
		E("%s(%d): FW update error code = %d\n", __func__, __LINE__, ret);
...
#if defined(HX_ZERO_FLASH)
	D("%s: It will update fw after esd event in zero flash mode!\n",
		__func__);
...
static int himax_wake_event_parse(struct himax_ts_data *ts, int ts_status)
{
	...
	for (i = 0; i < GEST_PTLG_ID_LEN; i++) {
		...
		if (buf[i] == gesture_flag) {
			...
			D("%s: ID START at %x , value = 0x%2X skip the event\n",
					__func__, i, buf[i]);
			break;
		}
	}

	D("%s: Himax gesture_flag=%x check_FC=%d\n", 
	...
	&& buf[GEST_PTLG_ID_LEN + 1] == GEST_PTLG_HDR_ID2) {
		...
		D("%s: gest doornidate start. gest_len=%d\n", 
			__func__, gest_len);
		...
		while (i < (gest_len + 1) / 2) {
			...
			i++;
			D("%s: gest_pt_x[%d]=%d,gest_pt_y[%d]=%d\n",
				__func__,
				gest_pt_cnt,
		...
		if (gest_pt_cnt) {
			...
			D("%s: gest_point x_min=%d,x_max=%d,y_min=%d,y_max=%d\n",
				__func__, tmp_min_x, tmp_max_x, tmp_min_y, tmp_max_y);
...
static void himax_wake_event_report(void)
{
	...
	if (KEY_EVENT) {
		D("%s: SMART WAKEUP KEY event %d press\n", __func__, KEY_EVENT);
		input_report_key(private_ts->input_dev, KEY_EVENT, 1);
		input_sync(private_ts->input_dev);
		D("%s: SMART WAKEUP KEY event %d release\n",
				__func__, KEY_EVENT);
		input_report_key(private_ts->input_dev, KEY_EVENT, 0);
		input_sync(private_ts->input_dev);
#if defined(HX_GESTURE_TRACK)
		D("%s: gest_start_x=%d,start_y=%d,end_x=%d,end_y=%d\n",
			__func__,
			gest_start_x,
			gest_start_y,
			gest_end_x,
			gest_end_y);
		D("%s: gest_width=%d,height=%d,mid_x=%d,mid_y=%d\n",
			__func__,
			gest_width,
			gest_height,
			gest_mid_x,
			gest_mid_y);
		D("%s: gest_up_x=%d,up_y=%d,down_x=%d,down_y=%d\n",
			__func__,
			hx_gesture_coor[8],
			hx_gesture_coor[9],
			hx_gesture_coor[10],
			hx_gesture_coor[11]);
		D("%s: gest_left_x=%d,left_y=%d,right_x=%d,right_y=%d\n",
			__func__,
			hx_gesture_coor[12],
			hx_gesture_coor[13],
			hx_gesture_coor[14],
...
void himax_cable_detect_func(bool force_renew)
{
	...
	if (ts->cable_config) {
		if ((connect_status != ts->usb_connected) || force_renew) {
			...
			D("%s: Cable status change: 0x%2.2X\n", __func__,
					ts->usb_connected);
		}
	}
}
...
#endif
__attribute__ ((unused)) static int himax_ts_event_check(struct himax_ts_data *ts,
...
/* SMWP */
	...
	if (g_ts_dbg != 0)
		D("%s: Now Path=%d, Now status=%d, length=%d\n",
				__func__, ts_path, ts_status, length);
 ...
 *			D("%s: [HIMAX TP MSG]: ESD event checked
				- ALL Zero in ZF.\n", __func__);
 *		} else {
 *			D("%s: [HIMAX TP MSG]: Status check pass in ZF.\n", __func__);
 *		}
 *	}
 *#endif
 */
...
skip_pen_operation:
	...
	for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
		...
		&& y <= ts->pdata->abs_y_max) {
			...
			/*D("%s: g_target_report_data->x[loop_i]=%d,*/
	...
	case HX_REPORT_COORD_RAWDATA:
		...
		if (debug_data != NULL) {
			if (debug_data->fp_set_diag_cmd(ic_data, hx_touch_data))
				D("%s: raw data_checksum not match\n", __func__);
		} else {
...
static void himax_finger_leave(struct himax_ts_data *ts)
{
	...
	if (himax_palm_detect(hx_touch_data->hx_coord_buf) == PALM_REPORT) {
		D("%s: HX_PALM_REPORT KEY power event press\n", __func__);
		input_report_key(ts->input_dev, KEY_POWER, 1);
		input_sync(ts->input_dev);
		msleep(100);

		D("%s: HX_PALM_REPORT KEY power event release\n", __func__);
...
int himax_chip_common_init(void)
{
	...
	if (idx >= 0) {
		if (isEmpty(idx) != 0) {
			D("%s: no chip registered, please insmod ic.ko!\n",
					__func__);
		...
		for (i = 0; i < entry->hx_ic_dt_num; i++) {
			if (entry->core_chip_dt[i].fp_chip_detect != NULL) {
				if (entry->core_chip_dt[i].fp_chip_detect()) {
					D("%s: chip found! list_num=%d\n",
							__func__, i);
					goto found_hx_chip;
				} else {
					D("%s: num=%d,chip NOT found! go Next\n",
							__func__, i);
					continue;
				}
			}
...
int himax_chip_common_suspend(struct himax_ts_data *ts)
{
	...
	if (debug_data != NULL && debug_data->flash_dump_going == true) {
		D("%s: Flash dump is going, reject suspend\n",
				__func__);
		goto END;
	}
...
int himax_chip_common_resume(struct himax_ts_data *ts)
{
	...
	if (!ts->SMWP_enable) {
#endif
		D("%s: It will update fw after resume in zero flash mode!\n",
			__func__);
